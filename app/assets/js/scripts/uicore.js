...
window.eval = global.eval = function () {
    throw new Error('Извините, это приложение не поддерживает window.eval().')
}
    ...
    ipcRenderer.on('autoUpdateNotification', (event, arg, info) => {
        switch(arg){
            case 'checking-for-update':
                loggerAutoUpdater.log('Проверка на обновление..')
            ...
            case 'update-downloaded':
                loggerAutoUpdaterSuccess.log('Обновить ' + info.version + ' готов к установке.')
                settingsUpdateButtonStatus('Установите сейчас', false, () => {
                    if(!isDev){
            ...
            case 'update-not-available':
                loggerAutoUpdater.log('Новое обновление не найдено.')
                settingsUpdateButtonStatus('Проверить наличие обновлений')
                break
            case 'ready':
            ...
            case 'realerror':
                if(info != null && info.code != null){
                    if(info.code === 'ERR_UPDATER_INVALID_RELEASE_FEED'){
                        ...
                        loggerAutoUpdater.error('Ошибка при проверке обновлений..', info)
