...
document.getElementById('launch_button').addEventListener('click', function(e){
    ...
    if(jExe == null){
        ...
        jg._validateJavaBinary(jExe).then((v) => {
            loggerLanding.log('Мета версия Java', v)
            if(v.valid){
                dlAsync()
            } else {
                asyncSystemScan(mcVersion)
...
function updateSelectedAccount(authUser){
    ...
    if(authUser != null){
        ...
        if(authUser.uuid != null){
            document.getElementById('avatarContainer').style.backgroundImage = `url('https://adderiy.com/api/3d/&user=${authUser.displayName}&headOnly=true')`
        }
    }
...
const refreshMojangStatuses = async function(){
    loggerLanding.log('Обновление статуса Adderiy..')
    ...
    try {
        ...
        loggerLanding.warn('Невозможно обновить статус сервиса Adderiy.')
...
const refreshServerStatus = async function(fade = false){
    ...
    try {
        ...
        loggerLanding.warn('Невозможно обновить состояние сервера, предполагая, что вы в автономном режиме.')
        loggerLanding.debug(err)
    }
    if(fade){
...
let mojangStatusListener = setInterval(() => refreshMojangStatuses(true), 150000)
...
function asyncSystemScan(mcVersion, launchAfter = true){
    ...
    sysAEx.on('message', (m) => {
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    'Совместимая установка<br>Java не найдена',
                    'Для того, чтобы присоединиться к AdderiyCraft, вам нужна 64-битная установка Java 8. Вы хотите, чтобы мы установили копию? Устанавливая, вы принимаете <a href="http://www.oracle.com/technetwork/java/javase/terms/license/index.html">Oracle\'s лицензионное соглашение</a>.',
                    'Установить Java',
                    'Установить вручную'
                )
                setOverlayHandler(() => {
                    setLaunchDetails('Подготовка Java Скачать..')
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            'Требуется Java<br>запустить',
                            'Для запуска требуется 8-битная версия Java 8.<br><br>Please refer to our <a href="https://github.com/dscalzi/HeliosLauncher/wiki/Java-Management#manually-installing-a-valid-version-of-java">Java Management Guide</a> for instructions on how to manually install Java.',
                            'Я понимаю',
                            'Вернутся'
                        )
                        setOverlayHandler(() => {
    ...
    setLaunchDetails('Проверка системной информации..')
...
function dlAsync(login = true){
    ...
    aEx.on('error', (err) => {
        loggerLaunchSuite.error('Ошибка при запуске', err)
        showLaunchFailure('Ошибка при запуске', err.message || 'See console (CTRL + Shift + i) for more details.')
    })
    aEx.on('close', (code, signal) => {
        if(code !== 0){
            loggerLaunchSuite.error(`AssetExec exited with code ${code}, assuming error.`)
            showLaunchFailure('Ошибка при запуске', 'See console (CTRL + Shift + i) for more details.')
        }
    })
        ...
        if(m.context === 'validate'){
            switch(m.data){
                ...
                case 'libraries':
                    ...
                    setLaunchDetails('Проверка различной целостности файла..')
                    break
                case 'files':
                    setLaunchPercentage(100, 100)
                    loggerLaunchSuite.log('Проверка файла завершена.')
                    setLaunchDetails('Скачивание файлов..')
                    break
            }
        } else if(m.context === 'progress'){
            switch(m.data){
            ...
            switch(m.data){
                case 'download':
                    ...
                    if(m.error.code === 'ENOENT'){
                        showLaunchFailure(
                            'Ошибка загрузки',
                            'Не удалось подключиться к файловому серверу. Убедитесь, что вы подключены к Интернету и попробуйте снова.'
                        )
                    } else {
                        showLaunchFailure(
                            'Ошибка загрузки',
            ...
            if(m.result.forgeData == null || m.result.versionData == null){
                loggerLaunchSuite.error('Ошибка во время проверки:', m.result)

                loggerLaunchSuite.error('Ошибка при загрузке', m.result.error)
            ...
            if(login && allGood) {
                ...
                const gameStateChange = function(data){
                    ...
                    if(SERVER_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Изучение Царства!')
                    } else if(GAME_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Sailing to Adderiy!')
                    }
                }
