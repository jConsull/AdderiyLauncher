    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                isLastAccount = true
                setOverlayContent(
                    'Предупреждение<br>Это ваш последний аккаунт',
                    'Чтобы использовать панель запуска, вы должны войти как минимум в одну учетную запись. После этого вам нужно будет снова войти в систему. <br> <br> Вы уверены, что хотите выйти?',
                    'Я уверен',
                    'Отмена'
                )
                setOverlayHandler(() => {
...
function refreshAuthAccountSelected(uuid){
    Array.from(document.getElementsByClassName('settingsAuthAccount')).map((val) => {
        ...
        if(uuid === val.getAttribute('uuid')){
            ...
            selBtn.innerHTML = 'Выбранный аккаунт &#10004;'
        } else {
            if(selBtn.hasAttribute('selected')){
                selBtn.removeAttribute('selected')
            }
            selBtn.innerHTML = 'Выберите аккаунт'
        }
    })
}
    ...
    authKeys.map((val) => {
        ...
        authAccountStr += `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            <div class="settingsAuthAccountLeft">
                <img class="settingsAuthAccountImage" alt="${acc.displayName}" src="https://adderiy.com/api/3d/&user=${acc.displayName}">
            </div>
            <div class="settingsAuthAccountRight">
                <div class="settingsAuthAccountDetails">
                    <div class="settingsAuthAccountDetailPane">
                        <div class="settingsAuthAccountDetailTitle">Логин</div>
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Выбранный аккаунт &#10004;' : '>Выбрать аккаунт'}</button>
                        ...
                        <div class="settingsModMainWrapper">
                            ...
                            <div class="settingsModDetails">
                                ...
                                <div class="settingsDropinRemoveWrapper">
                                    <button class="settingsDropinRemoveButton" remmod="${dropin.fullName}">Удалить</button>
                                </div>
                            </div>
                        </div>
                        <label class="toggleSwitch">
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    `Не удалось удалить<br>Drop-in Мод ${fullName}`,
                    'Убедитесь, что файл не используется и попробуйте снова.',
                    'Хорошо'
                )
        ...
        if(dropinUI != null){
            ...
            if(DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled){
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if(!isOverlayVisible()){
                        setOverlayContent(
                            'Не удалось переключить<br>Один или несколько дроп-ин модов',
                            err.message,
                            'Хорошо'
                        )
                        setOverlayHandler(null)
        ...
        <div class="serverListingDetails">
            ...
            <div class="serverListingInfo">
                ...
                ${serv.isMainServer() ? `<div class="serverListingStarWrapper">
                    ...
                    <span class="serverListingStarTooltip"Главный сервер</span>
                </div>` : ''}
            </div>
        </div>
    `
...
function populateVersionInformation(version, valueElement, titleElement, checkElement){
    ...
    if(isPrerelease(version)){
        ...
        titleElement.innerHTML = 'Стабильный релиз'
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/warfare644/AdderiyLauncher/releases.atom',
        success: (data) => {
        ...
        settingsAboutChangelogText.innerHTML = 'Не удалось загрузить заметки о выпуске.'
    })
}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `Новый ${isPrerelease(data.version) ? 'Pre-релиз' : 'Релиз'} доступный`
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('Скачать с Adderiy<span style="font-size: 10px;color: gray;text-shadow: none !important;">Закройте лаунчер и запустите dmg для обновления.</span>', false, () => {
        ...
        settingsUpdateTitle.innerHTML = 'Вы запускаете последнюю версию'
